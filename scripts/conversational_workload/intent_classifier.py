...
def classify_ai_slot(ai_message: str, flow: str = "pcp") -> str:  # noqa: C901
    ...
    if any(
        phrase in ai_lower
        for phrase in ["5-digit zip", "five-digit zip", "new zip", "provide.*zip"]
    ):
        return "zip_update"
    ...
    if "fax number" in ai_lower and any(
        phrase in ai_lower
        for phrase in ["is that correct", "is this correct", "on file", "to confirm"]
    ):
        return "fax_confirm"
    ...
    if "benefits" in ai_lower and any(
        phrase in ai_lower
        for phrase in ["would you like to also", "also get the benefits", "hear about"]
    ):
