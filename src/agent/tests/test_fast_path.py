...
def test_full_provider_flow_routing() -> None:
    ...
    assert get_fast_path_route(state_ps_complete) is None, (
        "provider_search complete is graph-routed to delivery_management_agent; fast_path must not intercept"
    )
...
# ---------------------------------------------------------------------------
# Closure loop guard (Bug 2)
# ---------------------------------------------------------------------------


def test_closure_agent_complete_does_not_re_route_to_closure() -> None:
    """closure_agent completing must not be routed back to itself."""
    state = _state(
        member_status_verify=True,
        active_agent="closure_agent",
        last_agent_signal={"status": "complete", "closure_requested": True},
    )
    result = get_fast_path_route(state)
    assert result != "closure_agent", "fast_path must not re-route closure_agent to itself"


def test_follow_up_closure_still_routes_to_closure_agent() -> None:
    """follow_up signalling closure must still reach closure_agent."""
    state = _state(
        member_status_verify=True,
        active_agent="follow_up_agent",
        last_agent_signal={"status": "complete", "closure_requested": True},
