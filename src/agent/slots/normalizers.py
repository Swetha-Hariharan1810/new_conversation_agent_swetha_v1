...
def _spoken_date_to_mdy(text: str) -> str | None:
    ...
    Handles both hyphenated compound ordinals ('twenty-fifth') and
    space-separated compound ordinals ('twenty fifth') as returned by the
    extraction LLM, so that dates like 'february twenty fifth nineteen
    ninety two' are parsed correctly as 02/25/1992.
    ...
    # Find day word.
    #
    # Check compound two-word forms BEFORE single-word ordinals.
    # The LLM returns spoken compound ordinals without hyphens, e.g.:
    #   "twenty fifth"  (not "twenty-fifth")
    #   "thirty first"  (not "thirty-first")
    #
    # Without the compound check, "twenty" falls through to `remaining` and
    # "fifth" is then matched as the standalone ordinal 5 — yielding day=5
    # instead of the correct day=25, and leaving "twenty" in the year tokens
    # which makes year parsing fail entirely.
    #
    # Priority:
    #   1. compound: tens-word + unit-ordinal ("twenty" + "fifth" → 25)
    #   2. single:   any key in _ORDINAL_TO_NUM ("twelfth" → 12, "twentieth" → 20)
    #   3. numeric:  bare 1-2 digit token, optionally with ordinal suffix
    #                ("2" → 2, "2nd" → 2). The 1-2 digit bound means a
    #                4-digit year token can never be consumed as the day;
    #                it falls through to `remaining` and is parsed as year.
    ...
    while i < len(words_without_month):
        ...
        if day_num is None:
            # Try compound ordinal first: tens + unit (space-separated)
            if w in _TENS_WORDS and i + 1 < len(words_without_month):
                unit = _UNIT_ORDINALS.get(words_without_month[i + 1])
                if unit is not None:
                    day_num = _TENS_WORDS[w] + unit
                    i += 2
                    continue
            # Try single ordinal
            if w in _ORDINAL_TO_NUM:
                ...
                i += 1
                continue
            # Try numeric day, with optional ordinal suffix: "2", "2nd"
...
def normalize_dob(value: str | None) -> str:
    ...
    Handles:
      ...
      - Spoken date:  "April twelfth nineteen eighty eight"     → "04/12/1988"
      - Spoken date with space-separated compound ordinal:
    ...
    #    "February twenty fifth nineteen ninety two", "november second 1989")
