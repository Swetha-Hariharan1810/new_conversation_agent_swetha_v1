...
# follow_up_intent=new_intent (with detected_intent), clears domain state, sets
# the new call_intent, and signals COMPLETE carrying new_intent_detected. The
# fast-path then routes STRAIGHT to the new domain agent WITHOUT re-verifying the
# member (member_status_verify stays True). Both directions are covered.
# ──────────────────────────────────────────────────────────────────────────────
...
pcp_then_claim_new_intent = Scenario(
    ...
    + [
        ...
        # claim_adjustment_agent has taken over (no re-verification) and asks for
        # the adjustment reference number. The only adjustment fixture (42695817)
        # is linked to James (M310188), NOT Emily — so it can never resolve for
        # her member id → not-found → re-ask → deterministic escalation.
        "42695817",
        "42695817",
    ],
    expect=Expected(
        ...
        escalation_reason_contains="adjustment_reference_not_found",
        final_state={
            # The member was NEVER re-verified — they stay verified across the pivot.
            "member_status_verify": True,
            # follow_up_agent rewrote call_intent to the detected new intent, and
            # the claim flow ran under it.
            "call_intent": "claim_services",
        },
        # Proof the claim agent engaged after the pivot — it asks for the reference number.
        transcript_contains=[r"reference number"],
    ),
    notes=(
        "New-intent pivot PCP -> claim. Emily completes the provider flow, then "
        "during follow-up asks about a claim reprocessing. follow_up_agent "
        "classifies new_intent (detected_intent=claim_services), clears provider/"
        "follow-up state, sets call_intent=claim_services and signals COMPLETE "
        "with new_intent_detected; the fast-path routes straight to "
        "claim_adjustment_agent with NO re-verification (member_status_verify stays "
        "True). Terminal state is the documented adjustment_reference_not_found "
        "escalation, because reference 42695817 is linked to James (M310188), not "
        "Emily — so it can never resolve for her member id. This deterministically "
        "proves the routing + no-reverify mechanics without needing an Emily-scoped "
        "adjustment fixture. retries=1: the new_intent classification is LLM-driven."
    ),
)
...
claim_then_pcp_new_intent = Scenario(
    ...
    + [
        ...
        # provider_search_agent has taken over (no re-verification).
        "Primary Care Physician",  # provider type
    ...
    expect=Expected(
        ...
        escalated=False,
        final_state={
            # Verified once (claim flow); never re-verified across the pivot.
            "member_status_verify": True,
            # follow_up_agent rewrote call_intent to the detected new intent.
            "call_intent": "provider_services",
            # The provider flow ran to completion AFTER the pivot. provider_list_sent
            # is cleared by _handle_new_intent and then re-set True by the new flow.
            "provider_list_sent": True,
            "delivery_method": "email",
        },
    ),
    notes=(
        "New-intent pivot claim -> PCP. James completes the claim flow, then during "
        "follow-up asks to find an in-network doctor. follow_up_agent classifies "
        "new_intent (detected_intent=provider_services), clears claim/follow-up "
        "state, sets call_intent=provider_services and signals COMPLETE with "
        "new_intent_detected; the fast-path routes straight to provider_search_agent "
        "with NO re-verification. The provider flow then completes (email delivery), "
        "declining the benefits/Care-Coach offers (James has no M_Benefit_Plan__c, "
        "so declining avoids reading it). Marked mutating: James has no ZIP on file, "
        "so the provider flow writes one (02139); preflight snapshots/restores "
        "contact fields. retries=1: new_intent classification plus provider_type/"
