...
async def test_happy_two_questions_then_close() -> None:
    ...
    state = make_fu_state(
        messages=[_msg("assistant", "Is there anything else?"), _msg("user", user_1)]
    )
...
async def test_happy_family_deductible_question() -> None:
    ...
    assert "$2500" in get_response(result), (
        f"Expected $2500 in response: {get_response(result)!r}"
    )
...
async def test_happy_oop_max_question() -> None:
    ...
    assert is_ask(result), f"Expected ask, got next_node={result.get('next_node')!r}"
    response = get_response(result)
    assert "$3000" in response or "$7000" in response, (
        f"Expected OOP max amount in response: {response!r}"
    )
