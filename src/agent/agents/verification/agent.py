...
from agent.core.pending_intents import mark_resolved_by_target, next_open_correction
...
def phone_reopen_requested(state: State) -> bool:
    """True when an open invalidating correction targets the phone number.

    Pure helper (mirrors provider_search.zip_reopen_requested) so the re-entry
    guard is unit-testable without an LLM. When True, the verification agent must
    re-collect/confirm the phone number rather than taking the verified early
    exit and signalling complete.
    """
    item = next_open_correction(list(state.get("pending_intents") or []))
    return bool(item and item.get("target") == "phone_number")
...
def _spell_name(first: str, last: str) -> str:
    ...
    return f"{full_name}, {spelled_name}"
...
class VerificationAgent(BaseAgent):
    ...
    async def run(self, state: State) -> dict:  # noqa: C901
        # ── Re-entry: action a deferred phone_number correction (Task 10 pattern) ──
        # The orchestrator rewinds a "phone_number" correction here because
        # CORRECTION_OWNER maps it to verification_agent. This MUST run before the
        # verified early-exit below: a verified member who corrects their phone
        # mid-call would otherwise short-circuit to _signal_verified without
        # re-collecting the new number. Clear phone_confirmed and route to the
        # existing phone-confirmation collection path (claims flow / phone_confirmed
        # slot), and mark the correction resolved so it does not re-fire.
        # Gate on member_status_verify: the correction-rewind contract assumes a
        # verified member (the orchestrator only calls get_rewind_route when
        # member_verified, see fast_path). Without this guard an unverified member
        # carrying an open phone_number correction (capture_and_triage runs during
        # verification turns too) would re-enter here and call _finish_after_identity
        # with an incomplete identity `collected`, firing a Salesforce lookup
        # mid-identity-collection. Mirrors the verified early-exit guard below.
        if state.get("member_status_verify") and phone_reopen_requested(state):
            # Re-entry only clears the confirmation flag and routes to the phone
            # collection path. The correction is NOT marked resolved here: if the
            # re-ask never yields a confirmed phone (slot exhaustion, ambiguous
            # reply, call drop on the interrupt), resolving up front would let
            # safeguard 5b close the call with the correction never actioned — a
            # silently lost correction. Resolution happens in _signal_verified,
            # reached only after collect_post_lookup confirms the new phone
            # (mirrors provider_search, which resolves only after the SF write).
            state = {
                **state,
                "phone_confirmed": False,
                "phone_update_requested": False,
                "awaiting_slot": "phone_confirmed",
                # Force the claims post-lookup path, which collects/confirms the
                # phone number via the existing phone_confirmed slot. The member is
                # already verified, so _finish_after_identity skips the SF lookup
                # and goes straight to collect_post_lookup for the phone slot.
                # This is a LOCAL override only: the signal/interrupt results below
                # do not write call_intent back to LangGraph state, so the member's
                # real call_intent (e.g. provider_services) is preserved for
                # post-resolution routing.
                "call_intent": "claim_services",
            }
            logger.info("VerificationAgent: re-entry for deferred phone_number correction")
            collected = {k: (state.get(k) or "").strip() for k in IDENTITY_SLOT_ORDER}
            messages = list(state.get("messages") or [])
            return await self._finish_after_identity(state, collected, messages, "claim_services", None)
    ...
    def _signal_verified(self, state: State, collected: dict, member_record: dict | None) -> dict:
        ...
        if member_record:
            ...
            for field in [
                ...
                if val:
                    context_updates[field] = val
        # Resolve a deferred phone_number correction only now — the phone has been
        # collected and confirmed via collect_post_lookup (an unconfirmed phone
        # would have interrupted before reaching here). Resolving earlier (at
        # re-entry) risked closing the call with the correction never actioned.
        # signal_complete persists pending_intents_list() (self._pending_intents),
        # so update the instance list to override the OPEN list from_state seeded.
        if phone_reopen_requested(state):
            self._pending_intents = mark_resolved_by_target(
                self.pending_intents_list() or list(state.get("pending_intents") or []),
                "phone_number",
            )
