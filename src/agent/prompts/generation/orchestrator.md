...
The member may have follow-up questions about benefits, the provider list,
or care coach details. follow_up_agent handles them and routes to
closure_agent when the member signals they are done.
...
`signal_status = complete` AND `proactive_offer_available = false`
AND member raises a clarification about already-covered topics
→ `follow_up_agent`

follow_up_agent answers from session context only. It never calls
external tools and never escalates — it routes to closure_agent
when done.
