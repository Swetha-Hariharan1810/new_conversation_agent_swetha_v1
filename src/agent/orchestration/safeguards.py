...
from agent.core.pending_intents import all_in_scope_resolved
...
def apply_safeguards(
    ...
    # 5b. Cannot close while any in scope request is still open
    if decision.next_agent == "closure_agent" and not all_in_scope_resolved(
        list(state.get("pending_intents") or [])
    ):
        decision.next_agent = "escalation_agent"
        decision.reasoning = "Safeguard: open pending intents remain — escalation_agent"
