...
from agent.core.pending_intents import mark_independent_resolved_for_agent
...
class SignalsMixin:
    ...
    def ask_member(self, state: State, message: str) -> dict:
        ...
        # Voice any freshly-captured non-actionable side request (unsupported /
        # off_topic) exactly once, prepended to this turn's prompt. This keeps the
        # "never silently drop" promise without derailing the primary slot flow —
        # the helper only flips spoken status, never blocks closure or rewinds.
        ack = self.side_request_ack() if hasattr(self, "side_request_ack") else ""
        if ack and isinstance(message, str):
            message = f"{ack} {message}".strip() if message.strip() else ack
        result = {
        ...
        pending = self.pending_intents_list()
        if pending:
            result["pending_intents"] = pending
        return result
    ...
    def signal_complete(
        ...
        # Drain resolve/clear stage: an agent completing the work for a deferred
        # in_scope_independent request must mark that intent resolved, or
        # get_drain_route keeps re-routing here and all_in_scope_resolved never
        # clears. _build persists self.pending_intents_list(), so update it here.
        self._pending_intents = mark_independent_resolved_for_agent(
            self.pending_intents_list(), self.AGENT_NAME
        )
