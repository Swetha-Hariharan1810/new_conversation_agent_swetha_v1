...
> **⚠ Cost & data warning** — A full run is ~29 conversations × 8–17 LLM-driven turns
> each (several hundred LLM calls plus dozens of Salesforce queries). It costs real
> money, takes 15–45 minutes, and **4 scenarios write to Salesforce** (they are
...
`claim_email_change_on_upload`, `email_change_loop_in_notification`) are wrapped
in a `finally` block that restores the snapshotted values via
`update_member_contact` — even when the scenario fails mid-way.

## Scenario matrix

29 scenarios across 7 groups (see `scenarios.py` for the exact scripts):

- **A. PCP happy paths** — fax / email delivery, benefits declined, zip update, fax update.
