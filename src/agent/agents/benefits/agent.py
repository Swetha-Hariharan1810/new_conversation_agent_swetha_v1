...
class BenefitsAgent(BaseAgent):
    ...
    async def run(self, state: State) -> dict:  # noqa: C901
        ...
        # ── EARLY EXIT: already explained + offer already handled ────────────
        if state.get("benefits_explained") and state.get("care_coach_offered"):
        ...
        # ── PHASE A: Fetch benefits + explain + offer ────────────────────────
        ...
        # Normalise values: Salesforce may store as int or Decimal
        indv_ded   = str(benefits.get("individual_deductible", "") or "").rstrip(".0") or "0"
        fam_ded    = str(benefits.get("family_deductible", "")    or "").rstrip(".0") or "0"
        coins_pct  = str(benefits.get("coinsurance_percent", "")  or "").rstrip(".0") or "0"
        indv_oop   = str(benefits.get("individual_oop_max", "")   or "").rstrip(".0") or "0"
        fam_oop    = str(benefits.get("family_oop_max", "")       or "").rstrip(".0") or "0"
        ...
        # Persist benefits values into state so follow-up agent can reference them
