...
async def warm_llm_connections() -> None:
    ...
    Fires minimal concurrent calls on extraction and generation LLMs
    so the first real conversational turn does not pay TCP + TLS cost.

    Call this once from your ASGI lifespan or startup handler:
        asyncio.create_task(warm_llm_connections())
    ...
    """
    from agent.llm.config import get_extraction_llm, get_generation_llm

    async def _ping(llm) -> None:
        try:
    ...
    await _asyncio.gather(
        _ping(get_extraction_llm()),
        _ping(get_generation_llm()),
    )
