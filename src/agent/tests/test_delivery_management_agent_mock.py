...
def mock_extraction(monkeypatch) -> AsyncMock:
    fake_llm = MagicMock()
    monkeypatch.setattr(
        "agent.agents.delivery_management.agent.get_extraction_llm", lambda: fake_llm
    )
    mock = AsyncMock(return_value=EMPTY_ANSWERED)
    monkeypatch.setattr(
        "agent.agents.delivery_management.agent.extract_delivery_management_decision", mock
    )
    return mock
...
def mock_dispatch(monkeypatch) -> AsyncMock:
    ...
    monkeypatch.setattr(
        "agent.agents.delivery_management.agent.dispatch_provider_list", mock
    )
    return mock
...
def mock_fax_update(monkeypatch) -> AsyncMock:
    mock = AsyncMock(return_value=None)
    monkeypatch.setattr(
        "agent.agents.delivery_management.agent.update_fax_in_salesforce", mock
    )
    return mock
...
def mock_email_update(monkeypatch) -> AsyncMock:
    mock = AsyncMock(return_value=None)
    monkeypatch.setattr(
        "agent.agents.delivery_management.agent.update_email_in_salesforce", mock
    )
    return mock
...
async def test_benefits_offer_message_contains_provider_type(mock_extraction, mock_dispatch) -> None:
    ...
    assert any(
        word in get_response(result).lower()
        for word in provider_type_lower.split()
