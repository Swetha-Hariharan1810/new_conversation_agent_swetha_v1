...
def fax_email_reopen_requested(state: State) -> str | None:
    """Return the correction target ("fax"/"email") when an open invalidating
    correction targets a delivery contact, else None.

    Pure helper (mirrors provider_search.zip_reopen_requested) so the re-entry
    guard and the resolve point share one source of truth and stay unit-testable
    without an LLM.
    """
    item = next_open_correction(list(state.get("pending_intents") or []))
    if item and item.get("target") in ("fax", "email"):
        return item["target"]
    return None
...
class DeliveryManagementAgent(BaseAgent):
    ...
    async def run(self, state: State) -> dict:  # noqa: C901
        ...
        # Re entry to action a deferred contact correction. Clear the matching
        # confirmation so the existing fax or email flow collects the new value
        # and route to that slot. The correction is NOT marked resolved here: if
        # the re-ask never yields a confirmed new value (slot exhaustion,
        # ambiguous reply, call drop on the interrupt), resolving up front would
        # let safeguard 5b close the call with the correction never actioned — a
        # silently lost correction. Resolution happens in _proceed_to_dispatch,
        # reached only after the new fax/email is validated and written to
        # Salesforce (mirrors provider_search, which resolves only after the SF
        # write).
        reopen_target = fax_email_reopen_requested(state)
        # Only reset to the collection slot on FIRST entry into the re-open. While
        # we are already collecting ("fax"/"email") or confirming the read-back
        # ("fax_confirmed"/"email_confirmed") for this target, do NOT clobber
        # awaiting_slot — otherwise the read-back branch becomes unreachable and
        # the member's confirmation "yes" is never processed, looping forever.
        if reopen_target and current_awaiting not in (reopen_target, f"{reopen_target}_confirmed"):
            target = reopen_target
            state = {
                **state,
                "provider_list_sent": False,
                "benefits_offer_made": False,
                f"{target}_confirmed": False,
                "awaiting_slot": target,
            }
            current_awaiting = target
            delivery_method = target
        ...
        if current_awaiting == "fax_confirmed":
            ...
            if new_fax_raw:
                # Raw-utterance fallback: a fax dictated word-by-word can be
                # mis-transcribed by the extraction LLM (e.g. an extra digit) so
                # the extracted string fails 10-digit validation even though
                # normalizing the RAW utterance recovers the correct number.
                normalized = _resolve_contact(
                    new_fax_raw, last_user, normalize_fax_number, validate_fax_number
                )
                if normalized:
        ...
        if current_awaiting == "email_confirmed":
            ...
            if new_email_raw:
                # Raw-utterance fallback (mirrors fax): a spelled-out email can be
                # mangled by the extraction LLM even though normalizing the RAW
                # utterance recovers a valid address.
                normalized = _resolve_contact(new_email_raw, last_user, normalize_email, validate_email)
                if normalized:
    ...
    async def _proceed_to_dispatch(
        ...
        # Resolve a deferred fax/email correction only now — the new value has
        # been validated and persisted (or confirmed against the on-file value)
        # and the list has dispatched. Resolving earlier (at re-entry) risked
        # closing the call with the correction never actioned. ask_member (below,
        # via offer_result) persists pending_intents_list() (self._pending_intents),
        # so update the instance list to override the OPEN list from_state seeded.
        if reopen_target := fax_email_reopen_requested(state):
