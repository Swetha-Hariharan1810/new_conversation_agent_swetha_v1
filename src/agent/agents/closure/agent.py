...
class ClosureAgent(BaseAgent):
    ...
    async def run(self, state: State) -> dict:
        ...
        # The member has chosen to end the call. Resolve any open independent
        # side-intent (e.g. care_wellness/benefits, all served by the forward
        # flow) so the final state does not leave a handled request lingering
        # "open". Unsupported/off-topic items are left as-is — they are voiced by
        # side_request_ack and never block closure.
        from agent.core.pending_intents import IntentKind, mark_resolved_by_kind

        pending = list(state.get("pending_intents") or [])
        if pending:
            result["pending_intents"] = mark_resolved_by_kind(pending, IntentKind.IN_SCOPE_INDEPENDENT.value)
        return result
