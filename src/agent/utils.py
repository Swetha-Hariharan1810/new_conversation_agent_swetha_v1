...
def _resolve_contact(
    ...
    """Resolve a contact value (fax/email/phone) with a raw-utterance fallback.

    The extraction LLM sometimes mangles spoken contact details — e.g. a
    10-digit fax dictated word-by-word ("four one five ...") comes back as an
    11-digit string that fails validation. The deterministic normalizer applied
    to the RAW user utterance frequently recovers the correct value
    (``normalize_fax_number("four one five five five five three two one one")``
    == ``"4155553211"``).

    Strategy:
      1. Try the LLM-extracted value first; return it if it normalizes and
         validates (the common, fast path).
      2. Only when that FAILS, fall back to normalizing the raw utterance and
         return it if that validates.
      3. Return None when neither source yields a valid value, so the caller
         can count a failed attempt / re-ask exactly as before.

    Pure and side-effect free so it can be unit-tested without an LLM.
    """
    for candidate in (extracted_value, raw_text):
        if not candidate:
            continue
        normalized = normalizer(str(candidate))
        if not normalized:
            continue
        result = validator(normalized)
        valid = result.valid if hasattr(result, "valid") else bool(result)
        if valid:
            return normalized
    return None
