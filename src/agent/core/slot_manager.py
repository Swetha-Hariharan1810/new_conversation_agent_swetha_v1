        ...
        *,
        ...
        if state.get("awaiting_slot") == slot_name:
            ...
            if event_value == EventType.AMBIGUOUS.value:
                ...
                # classified as AMBIGUOUS by the extraction LLM because they
                # contain no valid slot value and no correction intent.
                # Detect these BEFORE burning retry budget so the member is
                # transferred immediately rather than hearing the same question
                # two more times.
                last_user = _last_user_msg(messages)
                if detect_cannot_provide(last_user):
                    self.logger.info(
                        "_collect_slot: cannot-provide detected (ambiguous path) — "
                        "escalating immediately",
                        extra={"slot": slot_name},
                    )
                    return None, self.signal_escalate(
