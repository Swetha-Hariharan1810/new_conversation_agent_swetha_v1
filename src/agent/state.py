...
class State(TypedDict):
    ...
    # Set to True once the member has confirmed or corrected the spelled-out
    # name readback. Prevents re-running the readback step on re-entry.
    name_confirmed: bool
    # Number of complete name-confirmation cycles attempted (each cycle =
    # one readback → member response → rejection). Compared against
    # MAX_NAME_CONFIRM_ATTEMPTS before escalating.
