...
MSG_ESCALATE = [
    ...
    (
        "I've had trouble verifying your account after a few tries — let me get a "
        "specialist to help. Please hold."
    ),
]
MSG_RESTART = [
    "Let me try that again — could I start with your first name?",
    "No problem — let's try once more. What's your first name?",
    "Let me start fresh — could I get your first name?",
]
MSG_OFFTOPIC_PREFIX = [
...
def redirect_off_topic(agent, state, collected, identity_pipeline):
    ...
    if not next_slot:
        next_prompt = "Let me finish verifying your account first."
    else:
...
def apply_corrections(agent, collected, state, decision: "WorkerResult | None"):
    ...
    if corrections.get("first_name"):
        collected["last_name"] = ""
    if corrections.get("member_id"):
        collected["dob"] = ""
