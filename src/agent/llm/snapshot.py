"""
snapshot.py — session snapshot for grounded, single-pass answering.

Serialises the relevant session state into a compact, readable text block that is
injected into a decode's user content, so the model can answer a member's
in-scope question directly from what is already known — no second round-trip and,
crucially, no fabrication (a value not in the snapshot cannot be answered).

This is the generalization of the snapshot follow_up used for its single-call
answer (``agents/follow_up/llm._build_session_snapshot``): the same builder now
also feeds the multi-intent TurnPlan decode (Phase 2), so a relevant side
question can be answered inside the same decode call.

Spoken-form requirement: every email address and website is rendered in fully
spoken words ("at" / "dot") via ``speak_email``/``speak_url``, so any answer the
model derives from the snapshot speaks them verbatim.
"""
...
def build_session_snapshot(state: State) -> str:  # noqa: C901
    ...
    Rules:
      ...
      - Emails and URLs appear in spoken-word form ("at" / "dot").
    """
    lines: list[str] = []
