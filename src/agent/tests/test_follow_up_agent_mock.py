...
async def test_closure_keywords_complete_coverage(mock_llm, keyword) -> None:
    ...
    state = make_fu_state(messages=[_msg("assistant", "Is there anything else?"), _msg("user", keyword)])
    result = await _run(state)
    assert is_closure(result), (
...
async def test_guard_abuse_escalates(mock_llm, monkeypatch) -> None:
    ...
    state = make_fu_state(messages=[_msg("assistant", "Anything else?"), _msg("user", "this is useless")])
    result = await _run(state)
    assert result.get("next_node") == "escalation_agent"
    mock_llm.assert_not_called()
...
async def test_guard_self_harm_escalates(mock_llm, monkeypatch) -> None:
    ...
    state = make_fu_state(messages=[_msg("assistant", "Anything else?"), _msg("user", "I want to end it")])
    result = await _run(state)
    assert result.get("next_node") == "escalation_agent"
    mock_llm.assert_not_called()
...
async def test_llm_intent_unsure_returns_nudge(mock_llm, monkeypatch) -> None:
    ...
    state = make_fu_state(messages=[_msg("assistant", "Is there anything else?"), _msg("user", "yes")])
    result = await _run(state)
    assert is_ask(result)
    response = get_response(result)
    assert any(w in response.lower() for w in ["summary", "recap", "specific", "details"])
...
async def test_extraction_failure_closure_keyword_fallback(mock_llm, monkeypatch) -> None:
    ...
    state = make_fu_state(messages=[_msg("assistant", "Is there anything else?"), _msg("user", "no thanks")])
    result = await _run(state)
    assert is_closure(result)
    mock_llm.assert_not_called()
