...
from agent.orchestration.rewind import get_drain_route, get_rewind_route
...
def get_fast_path_route(state: State) -> str | None:  # noqa: C901
    ...
    # Correction of a committed value takes priority over the normal forward
    # chain once safety and verification have been cleared: rewind to the
    # value's owner before anything else proceeds.
    if member_verified:
        rewind = get_rewind_route(state)
        if rewind:
            return rewind
    ...
    # Nothing else matched: drain a deferred independent request if one is open.
    if member_verified:
        drain = get_drain_route(state)
        if drain:
            return drain
