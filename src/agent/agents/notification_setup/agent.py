...
class NotificationSetupAgent(BaseAgent):
    ...
    async def run(self, state: State) -> dict:  # noqa: C901
        ...
        if current_awaiting == "phone_confirmed":
            ...
            if new_phone_raw:
                # Raw-utterance fallback: a spoken phone number can be mangled by
                # the extraction LLM even when normalizing the RAW utterance
                # recovers a valid 10-digit number.
                normalized = _resolve_contact(
                    new_phone_raw, last_user, normalize_phone_number, validate_phone_number
                )
                if normalized:
        ...
        if current_awaiting == "phone":
            ...
            if phone_raw:
                normalized = _resolve_contact(
                    phone_raw, last_user, normalize_phone_number, validate_phone_number
                )
                if normalized:
        ...
        if current_awaiting == "email_confirmed":
            ...
            if new_email_raw:
                # Raw-utterance fallback (mirrors phone): a spelled-out email can
                # be mangled by the extraction LLM even when normalizing the RAW
        ...
        if current_awaiting == "email":
            ...
            if email_raw:
                normalized = _resolve_contact(email_raw, last_user, normalize_email, validate_email)
                if normalized:
