...
_SLOT_LABELS: dict[str, str] = {
    ...
    "relationship": "whether they are the plan holder or subscriber",
    ...
    "intent": "determine whether the caller needs provider search assistance or claim support",
...
_FALLBACKS: dict[str, str] = {
    ...
    "OFFTOPIC_AGENT": "Happy to help with that shortly — for now, could I get your {slot_label}?",
...
async def generate_recovery_message(
    *,
    ...
    confirmed_slots: dict of slot names already confirmed this session (values are ignored).
    ...
    """
    history_text = "\n".join(build_history(last_messages, n=8))
    ...
    if slot_label_override:
        content_lines.append(f"Context:    {slot_label_override}")
    if user_utterance:
        content_lines.append(f"Caller just said: {user_utterance}")
    if caller_name:
        content_lines.append(f"Caller's name:    {caller_name}")
    if confirmed_slots is not None and len(confirmed_slots) > 0:
        filled = ", ".join(confirmed_slots.keys())
        content_lines.append(f"Already confirmed: {filled}")
    elif confirmed_slots is not None:
        content_lines.append("Already confirmed: nothing yet")
