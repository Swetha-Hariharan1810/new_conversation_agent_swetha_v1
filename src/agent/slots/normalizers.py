...
def normalize_yes_no(value: str | None) -> str:
    """Normalize yes/no responses to canonical "yes" or "no".

    Handles exact matches and prefix matching for conversational responses
    like "yes that's correct" → "yes" and "no I don't" → "no".
    Returns the original lowercased string if not recognized.
    """
    cleaned = _clean(value).lower()
    _YES = {"yes", "yeah", "yep", "yup", "sure", "ok", "okay", "correct", "right", "true"}
    _NO = {"no", "nope", "nah", "wrong", "incorrect", "false"}
    if cleaned in _YES:
        return "yes"
    if cleaned in _NO:
        return "no"
    if cleaned.startswith(("yes ", "yeah ", "yep ", "yup ")):
        return "yes"
...
_DELIVERY_METHOD_MAP: dict[str, str] = {
    "fax": "fax",
    "fax it": "fax",
    "send it to my fax": "fax",
    "fax number": "fax",
    "email": "email",
    "send it to my email": "email",
    "email address": "email",
    "e-mail": "email",
    "mail": "email",
}
...
def normalize_provider_type(value: str | None) -> str:
    ...
    # If nothing matched but the value is non-empty, return title-cased as-is
