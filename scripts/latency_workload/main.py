        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for i in range(iterations):
                for name, steps in scenarios.items():
                    tag = f"{name}-iter-{i + 1}"
                    fut = executor.submit(run_one_conversation, steps, tag=tag)
                    futures[fut] = name
            for fut in concurrent.futures.as_completed(futures):
                per_demo[futures[fut]].extend(fut.result())
    else:
        for i in range(iterations):
            for name, steps in scenarios.items():
                tag = f"{name}-iter-{i + 1}"
                latencies = run_one_conversation(steps, tag=tag)
...
def main(argv=None) -> int:
    ...
    per_demo_latencies = _run_all_scenarios(scenarios, args.iterations, args.parallel)
