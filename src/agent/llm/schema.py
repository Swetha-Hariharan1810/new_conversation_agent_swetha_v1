...
class WorkerResult(BaseModel):
    ...
    # Name of a previously committed value the caller now wants to change
    # (for example "zip_code", "fax", "email", "phone_number"). None when no
    # correction was requested.
    correction_target: Optional[str] = None
    # Additional intents the caller raised in the same turn, as short labels
    # or topic words (for example ["pharmacy"] or ["benefits_inquiry"]).
