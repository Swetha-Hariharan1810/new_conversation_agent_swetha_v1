...
from agent.core.pending_intents import (
    mark_independent_resolved_for_agent,
    mark_resolved_by_target,
...
def zip_reopen_requested(state: State) -> bool:
    """True when an open invalidating correction targets the ZIP code."""
    item = next_open_correction(list(state.get("pending_intents") or []))
    return bool(item and item.get("target") == "zip_code")


def build_invalidation_updates() -> dict:
    """State updates that mark a previously dispatched list as stale so the
    delivery step rebuilds and re dispatches it for the corrected ZIP."""
    return {
...
class ProviderSearchAgent(BaseAgent):
    ...
    async def run(self, state: State) -> dict:  # noqa: C901
        ...
        reopen_zip = zip_reopen_requested(state)
        if provider_type and zip_code_used and not reopen_zip:
            return self._signal_done(state, provider_type, zip_code_used)
        if reopen_zip:
            # A correction is pending: force the ZIP collection path and clear
            # the stored value so the agent re asks for the corrected ZIP.
            state = {**state, "zip_code_used": "", "zip_code": "", "awaiting_slot": "zip_code"}
            zip_code_used = ""
        ...
        if current_awaiting == "zip_code":
            ...
            if zip_reopen_requested(state):
                done.update(build_invalidation_updates())
                # Resolve on top of the list _signal_done already persisted so a
                # drained independent intent stays resolved alongside the zip fix.
                done["pending_intents"] = mark_resolved_by_target(
                    list(done.get("pending_intents") or state.get("pending_intents") or []),
                    "zip_code",
                )
            return done
            ...
            if new_zip_raw:
                ...
                if normalized and validate_zip_code(normalized).valid:
                    ...
                    if zip_reopen_requested(state):
                        ...
                        # Resolve on top of the list _signal_done already
                        # persisted so a drained independent intent stays
                        # resolved alongside the zip fix.
                        done["pending_intents"] = mark_resolved_by_target(
                            list(done.get("pending_intents") or state.get("pending_intents") or []),
                            "zip_code",
                        )
    ...
    def _signal_done(self, state: State, provider_type: str, zip_code_used: str) -> dict:
        ...
        """
        # Drain resolve: when provider search runs because a deferred
        # in_scope_independent "provider_services" request was drained, this is
        # the point the work is done — the list is built and we hand off only the
        # delivery channel question. No agent downstream (delivery_management →
        # benefits → care_wellness → follow_up) carries AGENT_NAME
        # "provider_search_agent", so unless we resolve the intent here it stays
        # open forever, all_in_scope_resolved never clears, and safeguard 5b
        # reroutes closure → escalation. ask_member persists
        # self.pending_intents_list(), so mark the instance list (mirrors
        # delivery_management's correction-resolve handling).
        self._pending_intents = mark_independent_resolved_for_agent(
