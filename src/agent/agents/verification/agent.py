...
def _spell_name(first: str, last: str) -> str:
    """
    Produce a letter-by-letter spelled form for voice readback.
    "Emily Carter"  →  "E-M-I-L-Y  C-A-R-T-E-R"
    Hyphens inside names (e.g. "O'Brien") are preserved.
    Double space between first and last gives a natural voice pause.
    """
    def _spell(word: str) -> str:
        return "-".join(ch.upper() for ch in word if ch.isalpha() or ch == "-")

    parts = [_spell(first)] if first else []
    if last:
        parts.append(_spell(last))
    return "  ".join(parts)
...
class VerificationAgent(BaseAgent):
    ...
    async def _handle_name_confirmation(
        self, state: State, messages: list, last_user: str
    ) -> dict:
        """
    ...
    async def _process_name_readback_response(
        self, state: State, messages: list, last_user: str
    ) -> dict:
        """
        ...
        if interrupt := await self.run_conversation_guards(
            state, user_text=last_user, result=result
        ):
            return interrupt
    ...
    async def _collect_name_correction(
        self, state: State, messages: list, last_user: str
    ) -> dict:
        """
    ...
    def _name_confirmed_proceed(self, state: State) -> dict:
        """
        Mark name confirmed and hand back to the main verification pipeline.

        Uses signal_complete with next_node overridden to self.AGENT_NAME so
        conditional_routing re-enters run() instead of going to the orchestrator.
        On re-entry, name_confirmed=True and awaiting_slot="" so the name gate
        is skipped and member_id collection begins immediately.
        """
        ...
        result["next_node"]    = self.AGENT_NAME
