...
async def extract_follow_up_decision(
    ...
    *,
    ...
    parked_followups: list[str] | None = None,
) -> FollowUpResult:
    """
    ...
    # Parked follow-ups (Phase 6): the member asked these earlier in the call
    # and was promised an answer once the flow finished. Prepend them so the
    # QUESTION machinery answers them now, from the session snapshot.
    if parked_followups and messages:
        parked_block = (
