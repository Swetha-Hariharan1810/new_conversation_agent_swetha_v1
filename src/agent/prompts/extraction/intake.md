...
unclear — use when the caller has not described any specific need.
  ...
  Ask: has the caller described a specific topic?
    ...
    Yes, and it is an appeal or other different insurance/healthcare topic → out_of_scope
...
EVENT TYPE: answered_with_followup
  ...
  When event_type = answered_with_followup, ALSO set:
    followup_query — the side question, condensed, verbatim-ish
    followup_disposition — nothing is confirmed at intake yet, so this is
      effectively park or decline only:
        "park"       — the question will be answerable LATER in this call
                       (notifications, benefits, provider results, claim
                       status, anything a later step covers):
                       "can I get email notifications" → park
        "decline"    — irrelevant to this call or never answerable here:
                       "do you speak Spanish", "what are your hours" → decline
        "answer_now" — ONLY for repeat/confirmation requests about the
                       intent the caller just stated ("did you get that",
                       "is that right"); never for information questions
  When event_type != answered_with_followup, set followup_disposition "none"
  and followup_query null.
...
Examples:
  ...
  "I want to appeal my claim"                → out_of_scope
  "I'd like to file an appeal"               → out_of_scope
  "I'm calling about an appeal"              → out_of_scope
  "grievance inquiry"                        → out_of_scope
  "I want to appeal a denial"                → out_of_scope
