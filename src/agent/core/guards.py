...
class ConversationGuardsMixin:
    ...
    async def run_conversation_guards(
        ...
        *,
        ...
        # Dialogue Manager: record any correction or additional intents this
        # turn before the guard ladder runs. This never blocks the primary
        # answer; it only accumulates pending_intents for later draining.
        try:
            self.capture_and_triage(state, result)
        except Exception:
            self.logger.exception("run_conversation_guards: capture_and_triage failed")
