...
_NON_MEMBER_NUMBERS = {
    "provider": "1-800-XXX-PROV",
    "employer_group": "1-800-XXX-EMPL",
    "other_carrier": "1-800-XXX-CARR",
}
...
class ConversationGuardsMixin:
    ...
    async def _generate_guard_response(self, state: State, guard: str) -> str:
        # Lazy import: core.guards → llm.response_generator → llm.config → core (via schema);
        ...
        attempt = slot_state.get("attempt_count", 0) if isinstance(slot_state, dict) else 0
    ...
    def _handle_non_member_caller(
        ...
        Asks if they want to be transferred — does not escalate immediately.
        Saves current awaiting_slot so conversation can resume if they decline.
        """
        number = _NON_MEMBER_NUMBERS.get(caller_type, "1-800-XXX-XXXX")
        label = caller_type.replace("_", " ")

        msg = (
            f"Thank you for letting me know. Our dedicated line for "
            f"{label}s is {number} where you'll get specialized support. "
            f"Would you like me to transfer you there, or is there "
            f"something I can help you with today?"
        )
        ...
        result["slot_before_caller_type_check"] = state.get("awaiting_slot", "")
        return result
        ...
        *,
        ...
        if result is not None and result.guard_confidence >= 0.7:
            ...
            if guard == "OFFTOPIC_GLOBAL":
                ...
                # return self.ask_member(state, pick(MSG_OFFTOPIC_GLOBAL))
