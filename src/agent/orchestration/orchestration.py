...
from agent.core.telemetry import count_open_in_scope, log_unaddressed_request
...
async def orchestrator(state: State) -> dict:
    ...
    log_unaddressed_request(state, count=count_open_in_scope(list(state.get("pending_intents") or [])))
