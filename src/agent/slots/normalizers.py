...
def _matches_whole_word(cleaned: str, terms: set[str]) -> bool:
    """True if any term matches as a whole word/phrase (word-boundary aware).

    Whole-word matching prevents substring shadowing — e.g. the plan_holder
    term "me" must NOT match inside "family member".
    """
    return any(re.search(r"\b" + re.escape(term) + r"\b", cleaned) for term in terms)
...
def normalize_caller_role(value: str | None) -> str:
    ...
    if _matches_whole_word(cleaned, _PLAN_HOLDER_TERMS) or _matches_whole_word(cleaned, _SUBSCRIBER_TERMS):
        return "plan_holder"
    if _matches_whole_word(cleaned, _DEPENDENT_TERMS):
        return "dependent"
    return ""
