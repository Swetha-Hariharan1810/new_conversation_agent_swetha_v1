...
`new_intent_detected` is set in flags → route directly to the domain agent
for that intent based on the `call_intent` field (see Priority 4 table).
Do NOT route back to `follow_up_agent`.
Do NOT re-verify — `member_verified` is already true.
Clear `new_intent_detected` after routing.
